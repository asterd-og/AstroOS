...
Tga* tgaLoad(unsigned char *ptr, int size)
{
    ...
    data = (unsigned int*)Heap::alloc((w*h+2)*sizeof(unsigned int));
    if(!data) return NULL;
        ...
        case 1:
            ...
            for(y=i=0; y<h; y++) {
                ...
                for(x=0; x<w; x++) {
                    ...
                    data[i++] = ((ptr[7]==32?ptr[j+3]:0xFF) << 24) | (ptr[j+2] << 16) | (ptr[j+1] << 8) | ptr[j];
                }
            }
            break;
        case 2:
            ...
            for(y=i=0; y<h; y++) {
                ...
                for(x=0; x<w; x++) {
                    data[i++] = ((ptr[16]==32?ptr[j+3]:0xFF) << 24) | (ptr[j+2] << 16) | (ptr[j+1] << 8) | ptr[j];
                    j += ptr[16]>>3;
                }
            }
            break;
        ...
        case 10:
            ...
            for(x=0; x<w*h && m<size;) {
                ...
                if(k > 127) {
                    ...
                    while(k--) {
                        if(!(i%w)) { i=((!o?h-y-1:y)*w); y++; }
                        data[i++] = ((ptr[16]==32?ptr[m+3]:0xFF) << 24) | (ptr[m+2] << 16) | (ptr[m+1] << 8) | ptr[m];
                    }
                    m += ptr[16]>>3;
                } else {
                    k++; x += k;
    ...
    Tga* ret = (Tga*)Heap::alloc(sizeof(Tga));
    ret->width = w;
    ret->height = h;
    ret->data = data;
    return ret;
