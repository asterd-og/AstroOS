...
void Window::create(char* name, int w, int h, int x, int y, bool borderless, bool transparent) {
    this->name = name;
    this->w = roundTo(w, 8);
    this->h = roundTo(h, 8);
    this->x = roundTo(x, 8);
...
void Window::draw() {
    ...
    for (int i = 0; i < this->h; i++)
        for (int j = 0; j < this->w; j++)
            if (this->transparent)
                Gfx.drawAlphaPixel(j + this->x, i + this->y, this->gc.backBuffer[i * this->w + j]);
            else
